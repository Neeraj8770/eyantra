...
void motors_pin_config(void) {
	...
	motors_dir_ddr_reg |= ((1 << motors_RB_pin)|(1 << motors_LB_pin)|(1 << motors_RF_pin)|(1 << motors_LF_pin));			// set **ONLY** motor pins as output
	motors_dir_port_reg &= ~((1 << motors_RB_pin)|(1 << motors_LB_pin)|(1 << motors_RF_pin)|(1 << motors_LF_pin));			// stop motors initially
}
...
void pwm_pin_config(void){
	...
	motors_pwm_ddr_reg |= ((1 << motors_pwm_L_pin)|(1 << motors_pwm_R_pin));		// set **ONLY** left and right channel pin as output
...
void motors_move_forward(void) {
	...
	motors_dir_port_reg = ((motors_dir_port_reg & (~((1 << motors_LB_pin)|(1 << motors_RB_pin))))|((1 << motors_LF_pin)|(1 << motors_RF_pin)));	
}
...
void timer_pwm_init(void) {
	...
	TCCR5A_reg = ((1 << WGM0_bit)|(1 << COMA1_bit)|(1 << COMB1_bit));
