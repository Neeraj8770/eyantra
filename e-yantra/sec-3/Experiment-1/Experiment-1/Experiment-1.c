...
void adc_init(){
	...
	ADCSRA_reg	= ((1 << ADEN_bit)|(1 << ADPS2_bit)|(1 << ADPS1_bit)|(1 << ADIF_bit));
	
	// In ADCSRB, disable Analog Comparator Multiplexer, MUX5 bit and ADC Auto Trigger Source bits
	ADCSRB_reg	&= ~((1 << ACME_bit)|(1 << ADTS2_bit)|(1 << ADTS1_bit)|(1 << ADTS0_bit)|(1 << MUX5_bit));
...
void select_adc_channel( unsigned char channel_num ){
	...
	ADMUX_reg |= (channel_num & (~((1<<5)|(1<<6)|(1<<7))));
	ADCSRB_reg |= ((channel_num & (1<<5)) == 0)? ADCSRB_reg : (1 << MUX5_bit);
}
